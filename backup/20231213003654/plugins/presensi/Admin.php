...
class Admin extends AdminModule
{
    ...
    public function getJamJaga($page = 1)
    {
        ...
        // pagination
        $totalRecords = $this->core->mysql('jam_jaga')
            ->like('shift', '%' . $phrase . '%')
    ...
    public function getJagaAdd()
    {
        ...
        $this->assign['dep_id'] = $this->core->mysql('departemen')->toArray();
        $this->assign['shift'] = $this->core->mysql('jam_masuk')->toArray();
    ...
    public function postJagaSave($id = null)
    {
        ...
        if (!$errors) {
            ...
            if (!$id) {
                $query = $this->core->mysql('jam_jaga')->save($_POST);
            } else {
                $query = $this->core->mysql('jam_jaga')->where('no_id', $id)->save($_POST);
            }
    ...
    public function getJadwalAdd()
    {
        ...
        $username = $this->core->getUserInfo('username', null, true);
        if ($this->core->getUserInfo('role') == 'admin') {
            $this->assign['id'] = $this->core->mysql('pegawai')
        ...
        if ($this->core->getUserInfo('role') == 'admin') {
            $this->assign['h1'] = $this->core->mysql('jam_masuk')->toArray();
        } else {
            $this->assign['h1'] = $this->core->mysql('jam_jaga')->where('dep_id', $this->core->getPegawaiInfo('departemen', $username))->toArray();
        }
        $this->assign['tahun'] = array('', '2020', '2021', '2022', '2023', '2024', '2025');
    ...
    public function getJadwalEdit($id, $bulan, $tahun)
    {
        ...
        $row = $this->core->mysql('jadwal_pegawai')->where('id', $id)->where('tahun', $tahun)->where('bulan', $bulan)->oneArray();
        if (!empty($row)) {
            $username = $this->core->getUserInfo('username', null, true);
    ...
    public function postJadwalSave($id = null)
    {
        ...
        if (!$errors) {
            ...
            if (!$id) {
                $query = $this->core->mysql('jadwal_pegawai')->save($_POST);
            } else {
                $query = $this->core->mysql('jadwal_pegawai')->where('id', $id)->where('tahun', $_POST['tahun'])->where('bulan', $_POST['bulan'])->save($_POST);
            }
    ...
    public function getRekap_Presensi($page = 1)
    {
        ...
        $pagination = new \Systems\Lib\Pagination($page, count($totalRecords), 20, url([ADMIN, 'presensi', 'rekap_presensi', '%d?awal='.$tgl_kunjungan.'&akhir='.$tgl_kunjungan_akhir.'&ruang='.$ruang.'&s='.$phrase]));
        $this->assign['pagination'] = $pagination->nav('pagination','5');
        $this->assign['totalRecords'] = $totalRecords;
        ...
        $rows = $this->db('rekap_presensi')
            ->select([
              ...
              'photo_pulang' => 'rekap_presensi.photo_pulang'
            ])
            ->join('pegawai','pegawai.id = rekap_presensi.id')
            ->where('jam_datang', '>=', $tgl_kunjungan.' 00:00:00')
            ->where('jam_datang', '<=', $tgl_kunjungan_akhir.' 23:59:59')
  ...
  public function getRekapEdit()
    {
        ...
        $dtg = urldecode($datang);
      	$plg = urldecode($pulang);
      	$dataedit = $this->db('rekap_presensi')->where('id',$id)->where('shift',$shift)->where('jam_datang',$dtg)->where('jam_pulang',$plg)->oneArray();
      	$this->assign['pgw'] = $this->db('pegawai')->where('id',$id)->oneArray();
        $this->assign['shift'] = $this->db('jam_masuk')->toArray();
      	$this->assign['edit'] = $dataedit;
      	$this->assign['jdtg'] = substr($datang,11);
      	$this->assign['jplg'] = substr($pulang,11);
        return $this->draw('rekap_presensi.form.html', ['rekapedit' => $this->assign]);
    }
  
  	public function postRekapEditSave()
    {
      $id = $_GET['id'];
      $shift = $_GET['shift'];
      $datang = $_GET['datang'];
      ...
      if($_POST['id'] == '' OR $_POST['shift'] == '' OR $_POST['tgl_datang'] == '' OR $_POST['jam_datang'] == '' OR $_POST['tgl_pulang'] == '' OR $_POST['jam_pulang'] == ''){
      	$this->notify('failure', 'Simpan gagal');
      } else {
        $cek = $this->db('rekap_presensi')->where('id',$id)->where('shift',$shift)->where('jam_datang',$dtg)->where('jam_pulang',$plg)->oneArray();
        if(!empty($cek)) {
          	$jam_jaga = $this->db('jam_jaga')->join('pegawai', 'pegawai.departemen = jam_jaga.dep_id')->where('pegawai.id', $_POST['id'])->where('jam_jaga.shift', $_POST['shift'])->oneArray();
          	$set_keterlambatan  = $this->db('set_keterlambatan')->toArray();
          	$terlambat1     = $set_keterlambatan[0]['terlambat1'];
          	
          	$jam_datang = $_POST['tgl_datang'].' '.$_POST['jam_datang'];
          	$jam_pulang = $_POST['tgl_pulang'].' '.$_POST['jam_pulang'];
          	
			if (empty($jam_jaga)){
            	$this->notify('failure', 'Departemen tidak ada shift tersebut');
            } else {
              $status = 'Tepat Waktu';
              $selisih = (strtotime(date('Y-m-d H:i:s', strtotime($jam_datang))) - strtotime(date('Y-m-d', strtotime($jam_datang)) . ' ' . $jam_jaga['jam_masuk']));
              
              if ($selisih >= ($terlambat1 * 60)) {
                $status = 'Terlambat I';
              }
              ...
              if ($selisih >= ($terlambat1 * 60)) {
                 $awal  = new \DateTime(date('Y-m-d', strtotime($jam_datang)) . ' ' . $jam_jaga['jam_masuk']);
                 $akhir = new \DateTime($jam_datang);
                 $diff = $akhir->diff($awal, true); // to make the difference to be always positive.
                 $keterlambatan = $diff->format('%H:%I:%S');
              }

              if ($jam_pulang < date('Y-m-d', strtotime($jam_pulang)) . ' ' . $jam_jaga['jam_pulang']) {
                  $status = $status  . ' & PSW';
              }

              $awal  = new \DateTime($jam_datang);
              $akhir = new \DateTime($jam_pulang);
              $diff = $akhir->diff($awal, true); // to make the difference to be always positive.
              $durasi = $diff->format('%H:%I:%S');

              $insert = $this->db('rekap_presensi')->where('id',$id)->where('shift',$shift)->where('jam_datang',$dtg)->where('jam_pulang',$plg)
                    ->save([
                      'id' => $id,
                      'shift' => $_POST['shift'],
        	...
        	$this->notify('failure', 'Data Salah');
        }
      }
      redirect(url([ADMIN, 'presensi', 'rekap_presensi']));
    }
    ...
    public function getCetakRekap()
    {
        ...
        $pasien = $this->core->mysql('rekap_presensi')
            ->select([
                'nama' => 'pegawai.nama',
                'departemen' => 'pegawai.departemen',
                'id' => 'rekap_presensi.id',
        ...
        foreach ($pasien as $hasil) {
            ...
            $hasil['efektif'] = $this->core->mysql('rekap_presensi')
                ->select([
                    'efektif' => 'CAST(rekap_presensi.durasi as TIME) - ' . $efektif,
                    'kurang' => 'CAST(rekap_presensi.durasi as TIME) - ' . $interval
                ])
    ...
    public function getPrint($phrase = null, $tanggal = null)
    {
        ...
        $rekap_presensi = $this->core->mysql('rekap_presensi')
            ->select([
                'nama' => 'pegawai.nama',
                'departemen' => 'pegawai.departemen',
                'id' => 'rekap_presensi.id',
    ...
    public function getPresensi($page = 1)
    {
        ...
        $this->assign['dep'] = $this->core->mysql('departemen')->toArray();
    ...
    {
        $cek = $this->core->mysql('temporary_presensi')->where('id', $id)->oneArray();
        $jam_jaga       = $this->core->mysql('jam_jaga')->join('pegawai', 'pegawai.departemen = jam_jaga.dep_id')->where('pegawai.id', $id)->where('jam_jaga.shift', $cek['shift'])->oneArray();
        ...
        if ($cek) {
            ...
            if ($ubah) {
                $presensi = $this->core->mysql('temporary_presensi')->where('id', $id)->oneArray();
    ...
    public function getBarcode($page = 1)
    {
        ...
        // pagination
        $totalRecords = $this->core->mysql('barcode')
            ->select('id')
            ->like('barcode', '%' . $phrase . '%')
            ->toArray();
    ...
    {
        $this->_addHeaderFiles();
        $row = $this->core->mysql('barcode')->oneArray($id);
        if (!empty($row)) {
            $this->assign['form'] = $row;
    ...
    public function postBarcodeSave($id = null)
    {
        ...
        if (!$errors) {
            ...
            if (!$id) {    // new
                $query = $this->core->mysql('barcode')->save($_POST);
            } else {        // edit
                $query = $this->core->mysql('barcode')->where('id', $id)->save($_POST);
            }
