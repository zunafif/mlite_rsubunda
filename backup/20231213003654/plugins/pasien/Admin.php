      ...
      foreach ($reg_periksa as $row) {
        ...
        foreach ($detail_pemberian_obat as $row_pemberian_obat) {
          $row_pemberian_obat['data_pemberian_obat'] = $this->core->mysql('detail_pemberian_obat')
            ...
            ->toArray();
          	$arry_no_obt = 0;
          	foreach($row_pemberian_obat['data_pemberian_obat'] as $dt_pemberian_obat){
            	$row_pemberian_obat['data_pemberian_obat'][$arry_no_obt] += $this->core->mysql('aturan_pakai')
                  	->where('kode_brng', $dt_pemberian_obat['kode_brng'])
                    ->where('no_rawat', $row_pemberian_obat['no_rawat'])
                    ->where('tgl_perawatan', $row_pemberian_obat['tgl_perawatan'])
                    ->where('jam', $row_pemberian_obat['jam'])
                  	->oneArray();
              	$arry_no_obt = $arry_no_obt + 1;
            }
