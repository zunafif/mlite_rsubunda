...
$("#display").on("click", ".sep", function(event){
  ...
  $.get(url,function(data) {
    ...
    if(!json_obj[0]) {
      ...
      if(no_ktp != json_obj[0].response.peserta.nik){
      	notif_verifikasi += 'NIK / NO KTP pasien tidak sama antara VCLAIM ('+json_obj[0].response.peserta.nik+') dengan SIMRS ('+no_ktp+'), \n';
      }
      
      if(no_ktp.length != 16){
      	notif_verifikasi += 'NIK / NO KTP pasien di SIMRS tidak berjumlah 16 digit, \n';
      }
      
      if(no_tlp != json_obj[0].response.peserta.mr.noTelepon){
      	notif_tlp += 'NO Telepon pasien tidak sama antara VCLAIM ('+json_obj[0].response.peserta.mr.noTelepon+') dengan SIMRS ('+no_tlp+'), \n';
      }
      
      if(no_tlp.length < 10 || no_tlp.length > 13){
      	notif_tlp += 'NO Telepon pasien di SIMRS tidak berjumlah 10 - 13 digit, \n';
      }
      
      if (notif_verifikasi != '') {
      	alert(notif_verifikasi)
      } else {
        if (notif_tlp != '') {
      		alert(notif_tlp)
      	}
...
$("#form_kontrol").on("click", "#simpan_kontrol", function(event){
  ...
  var keterangan      = $('textarea[name=keterangan_kontrol]').val();
