...
class Admin extends AdminModule
{
    ...
    public function getJamJaga($page = 1)
    {
        ...
        $rows = $this->core->mysql('jam_jaga')
            ->join('departemen','jam_jaga.dep_id = departemen.dep_id')
            ->like('jam_jaga.shift', '%' . $phrase . '%')
            ->orLike('jam_jaga.dep_id', '%' . $phrase . '%')
            ->offset($offset)
            ->limit($perpage)
            ->toArray();
    ...
    public function getRekap_Presensi($page = 1)
    {
        ...
        if (count($rows)) {
            foreach ($rows as $row) {
                ...
                $jam_datang = $this->core->mysql('rekap_presensi')
                    ->select([
                ...
                $row['efektif'] = $this->core->mysql('rekap_presensi')
                    ->select([
