...
class Admin extends AdminModule
{
    ...
    {
      $sub_modules = [
        ['name' => 'Radiologi', 'url' => url([ADMIN, 'radiologi', 'manage']), 'icon' => 'wheelchair', 'desc' => 'Data pasien rawat jalan'],
        ['name' => 'Laporan', 'url' => url([ADMIN, 'radiologi', 'pendaftaran']), 'icon' => 'wrench', 'desc' => 'Pengaturan dokter rawat jalan'],
      ];
...
// ====================================== LAPORAN RAD ====================================== 
    ...
    {
        $record = $this->core->mysql('periksa_radiologi')
            ->select([
    ...
    public function poliChartBaru()
    {
        ...
        $query = $this->core->mysql('reg_periksa')
            ...
            ->join('poliklinik', 'poliklinik.kd_poli = reg_periksa.kd_poli')
            ->where('tgl_registrasi', '>=', '2024-03-11')
    ...
    public function countCheck($table)
    {
        $date = date('Y-m-d');
    ...
    public function countYear($table)
    {
        $date = date('Y');
        $record = $this->core->mysql($table)
            ->select([
    ...
    public function countMonth($table)
    {
        $date = date('Y-m');
        $record = $this->core->mysql($table)
            ->select([
    ...
    public function countDrPerujukRad()
    {
        ...
        $query = $this->core->mysql('periksa_radiologi')
            ...
            ->where('periksa_radiologi.tgl_periksa', '2024-03-12')
            ->group(['reg_periksa.kd_poli'])
            ->desc('nm_poli');
    ...
    public function getRawatJalan()
    {
        ...
        $settings = htmlspecialchars_array($this->settings('manajemen'));
        $stats['getVisities'] = number_format($this->countVisite(),0,'','.');
        $stats['getLab'] = number_format($this->countCheck('periksa_radiologi'),0,'','.');
        $stats['getLabMonthly'] = number_format($this->countMonth('periksa_radiologi'),0,'','.');
        $stats['getLabYearly'] = number_format($this->countYear('periksa_radiologi'),0,'','.');
        ...
        $stats['percentDays'] = 0;
        if($this->countCheck('periksa_radiologi') != 0) {
            $stats['percentDays'] = number_format((($this->countCheck('periksa_radiologi')-$this->countLastCheck('periksa_radiologi','rad1'))/$this->countCheck('periksa_radiologi','rad1'))*100,0,'','.');
        }
        $stats['percentMonths'] = 0;
        if($this->countMonth('periksa_radiologi') != 0) {
            $stats['percentMonths'] = number_format((($this->countMonth('periksa_radiologi')-$this->countLastMonth('periksa_radiologi','rad1'))/$this->countMonth('periksa_radiologi','rad1'))*100,0,'','.');
        }
        $stats['percentYears'] = 0;
        if($this->countYear('periksa_radiologi') != 0) {
            $stats['percentYears'] = number_format((($this->countYear('periksa_radiologi')-$this->countLastYear('periksa_radiologi','rad1'))/$this->countYear('periksa_radiologi','rad1'))*100,0,'','.');
        }
