...
class Site extends SiteModule
{
    ...
    public function routes()
    {
        ...
        $this->route('addautojkn', '_getAntreanAddMjkn');
      ...
      public function _getAntreanAddMjkn()
      {
          echo '
          ...
          $checkAntrian = $this->core->mysql('mlite_antrian_referensi')->where('tanggal_periksa', $date)->where('status_kirim','Auto')->limit(10)->toArray();
          ...
          if($checkAntrian){
              ...
              $booking_reg = $this->core->mysql('booking_registrasi')->where('tanggal_periksa', $date)->where('no_rkm_medis', $q['no_rkm_medis'])->oneArray();
              $pasien = $this->core->mysql('pasien')->where('no_rkm_medis', $q['no_rkm_medis'])->oneArray();
              $maping_dokter_dpjpvclaim = $this->core->mysql('maping_dokter_dpjpvclaim')->where('kd_dokter', $booking_reg['kd_dokter'])->oneArray();
              $maping_poli_bpjs = $this->core->mysql('maping_poli_bpjs')->where('kd_poli_rs', $booking_reg['kd_poli'])->oneArray();
              $jadwaldokter = $this->core->mysql('jadwal')->where('kd_dokter', $booking_reg['kd_dokter'])->where('kd_poli', $booking_reg['kd_poli'])->where('hari_kerja', $hari)->oneArray();
              $no_urut_reg = substr($booking_reg['no_reg'], 0, 3);
              $minutes = 0;
              ...
              $kodebooking = $kode_ppk.convertNorawat($q['no_rawat']).''.$booking_reg['no_reg'];
              ...
              $data = [
                  ...
                  'norm' => $q['no_rkm_medis'],
                  'tanggalperiksa' => $booking_reg['tanggal_periksa'],
                  ...
                  'nomorantrean' => $maping_poli_bpjs['kd_poli_bpjs'].'-'.$booking_reg['no_reg'],
                  'angkaantrean' => $booking_reg['no_reg'],
                  'estimasidilayani' => strtotime($booking_reg['tanggal_periksa'].' '.$cek_kouta['jam_mulai']) * 1000,
                  'sisakuotajkn' => $jadwaldokter['kuota']-ltrim($booking_reg['no_reg'],'0'),
                  'kuotajkn' => intval($jadwaldokter['kuota']),
                  'sisakuotanonjkn' => $jadwaldokter['kuota']-ltrim($booking_reg['no_reg'],'0'),
              ...
              if($data['metadata']['code'] == 200 || $data['metadata']['code'] == 208){
                  ...
                  if($jenispasien == 'JKN') {
                  if(!$this->core->mysql('mlite_antrian_referensi')->where('tanggal_periksa', $booking_reg['tanggal_periksa'])->where('nomor_kartu', $nomorkartu)->oneArray()) {
                  $this->core->mysql('mlite_antrian_referensi')->save([
                      'tanggal_periksa' => $q['tanggal_periksa'],
    ...
    public function _resultHapusGagal()
    {
        $date = date('Y-m-d');
      	//$date = '2023-12-21';
        $HapusAdd = $this->core->mysql('mlite_antrian_referensi')->where('tanggal_periksa',$date)->where('status_kirim','Gagal')->notlike('keterangan', 'data nik  belum sesuai.')->notlike('keterangan', 'data nohp  belum sesuai.')->delete();
    ...
    //   foreach ($tab_sep as $value) {
      ...
      $no_rawat = '2023/10/23/000417';
      $dokter = '13051951';
      $sep = '0156R0041223V002564';
