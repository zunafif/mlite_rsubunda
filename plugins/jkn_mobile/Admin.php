...
class Admin extends AdminModule
{
    ...
    public function getQrAntrol(){
        ...
        $sql = "SELECT * FROM bridging_sep WHERE tglsep = '$tanggal' AND jnspelayanan = '2' AND kdpolitujuan NOT IN ('IGD','HDL','IRM')";
        ...
        foreach ($sep_terbit as $valuex) {
            $nomor_referensi = '';
              //$cari_antrol = $this->core->mysql('mlite_antrian_referensi')->where('tanggal_periksa',$tanggal)->where('kodebooking','!=','')->where('status_kirim','Sudah')->oneArray();
          	$cari_antrol = $this->core->mysql('mlite_antrian_referensi')->where('tanggal_periksa',$tanggal)->where('nomor_referensi',$valuex['no_rujukan'])->where('kodebooking','!=','')->where('status_kirim','Sudah')->oneArray();
            if (!$cari_antrol) {
                if ($valuex['noskdp'] != '') {
                    $cari_antrol_kontrol = $this->core->mysql('mlite_antrian_referensi')->where('tanggal_periksa',$tanggal)->where('nomor_referensi',$valuex['noskdp'])->where('kodebooking','!=','')->where('status_kirim','Sudah')->oneArray();
					//$cari_antrol_kontrol = $this->core->mysql('mlite_antrian_referensi')->where('tanggal_periksa',$tanggal)->where('kodebooking','!=','')->where('status_kirim','Sudah')->oneArray();
