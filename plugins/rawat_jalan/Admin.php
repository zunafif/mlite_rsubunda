...
class Admin extends AdminModule
{
  public function init()
  {
    ...
    public function anyManage()
    {
        ...
        //$cek_vero = $this->core->mysql('mlite_veronisa')->where('tgl_registrasi', '2024-03-02')->toArray();
    ...
    public function _Display($tgl_kunjungan, $tgl_kunjungan_akhir, $status_periksa='', $status_bayar='')
    {
        ...
        foreach ($rows as $row) {
          ...
          $cek_vero = $this->core->mysql('mlite_veronisa')->where('no_rawat', $row['no_rawat'])->oneArray();
          $row['cek_vero'] = isset_or($cek_vero['no_rawat']);
    ...
    public function anyBooking($page = 1)
    {
      ...
      // pagination
      $totalRecords = $this->core->mysql()->pdo()->prepare("SELECT booking_registrasi.no_rkm_medis FROM booking_registrasi, pasien, dokter WHERE booking_registrasi.no_rkm_medis = pasien.no_rkm_medis AND booking_registrasi.kd_dokter = dokter.kd_dokter AND (dokter.nm_dokter LIKE ? OR pasien.nm_pasien LIKE ?) AND booking_registrasi.tanggal_periksa BETWEEN '$start_date' AND '$end_date'");
      ...
      $query = $this->core->mysql()->pdo()->prepare("SELECT booking_registrasi.*, pasien.nm_pasien, pasien.alamat, pasien.no_tlp, dokter.nm_dokter, poliklinik.nm_poli, penjab.png_jawab, pasien.no_peserta FROM booking_registrasi, pasien, dokter, poliklinik, penjab WHERE booking_registrasi.no_rkm_medis = pasien.no_rkm_medis AND booking_registrasi.kd_dokter = dokter.kd_dokter AND booking_registrasi.kd_poli = poliklinik.kd_poli AND booking_registrasi.kd_pj = penjab.kd_pj AND (dokter.nm_dokter LIKE ? OR pasien.nm_pasien LIKE ?) AND booking_registrasi.tanggal_periksa BETWEEN '$start_date' AND '$end_date' LIMIT $perpage OFFSET $offset");
    ...
    public function postSaveKontrol()
    {
      ...
      if ($query) {
        ...
        $_next_no_reg = sprintf('%03s', ($max_reg[0] + 1));
        $_next_no_reg_10 = sprintf('%03s', (1 + 10));

        if($no_reg >= 0 && $no_reg <= 10){
          if($max_no_reg > 10) {
            $no_reg = $_next_no_reg;
          } else {
            $no_reg = $_next_no_reg_10;
          }
        }
    ...
    public function getAntrian()
    {
      ...
      $sep = $this->core->mysql('bridging_sep')->where('no_rawat', $_GET['no_rawat'])->oneArray();
      
      $potensi_prb = $this->core->mysql('bpjs_prb')->where('no_sep', $sep['no_sep'])->oneArray();
      $data_sep['potensi_prb'] = $potensi_prb['prb'];
      
      echo $this->draw('antrian.html', ['rawat_jalan' => $rawat_jalan, 'data_sep' => $data_sep, 'sep' => $sep]);
      exit();
    }
