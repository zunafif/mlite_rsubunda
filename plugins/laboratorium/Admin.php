...
class Admin extends AdminModule
{
  ...
  public function getLihatHasil($id){
      $settings = $this->settings('settings');
      $this->tpl->set('settings', $this->tpl->noParse_array(htmlspecialchars_array($settings)));
    ...
    foreach ($reg_periksa as $data) {
        ...
        $pemeriksaan_laboratorium[$no] = $data;
          ...
          foreach ($rows_pemeriksaan_laboratorium as $value) {
            $value['detail_periksa_lab'] = $this->core->mysql('detail_periksa_lab')
              ...
              ->toArray();
            $pemeriksaan_laboratorium[$no]['periksa_lab'][] = $value;
          }
      ...
      $pemeriksaan_laboratorium[$no]['petugas_lab'] = $petugas_lab;
      
      $no = $no + 1;
    }
      
      echo $this->draw('lihathasil.html', [
        'pasien' 	=> $pasien,
        'instansi'	=> $instansi,
