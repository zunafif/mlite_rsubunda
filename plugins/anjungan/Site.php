       ...
       default:
        ...
        case "get-skdp":
          if(!empty($_POST['no_rkm_medis'])){
              ...
              $query = $this->core->mysql('booking_registrasi')
                ...
                ->where('booking_registrasi.kd_poli', $kd_poli)
                ->desc('booking_registrasi.tanggal_periksa')
                ->oneArray();
            
             $skdp = $this->core->mysql('skdp_bpjs')
                ->where('skdp_bpjs.no_rkm_medis', $no_rkm_medis)
               	->where('skdp_bpjs.tanggal_datang', $query['tanggal_periksa'])
                ->desc('skdp_bpjs.tanggal_datang')
                ->oneArray();
            
              if(!empty($query) && !empty($skdp)){
                  $data['status'] = 'ok';
                  $data['result'] = $query;
                  $data['result']['no_antrian'] = $skdp['no_antrian'];
                  $data['result']['tanggal_datang'] = $skdp['tanggal_datang'];
                  $data['result']['tanggal_rujukan'] = $skdp['tanggal_rujukan'];
                  $data['result']['diagnosa'] = $skdp['diagnosa'];
                  $data['result']['terapi'] = $skdp['terapi'];
                  $data['result']['alasan1'] = $skdp['alasan1'];
                  $data['result']['alasan2'] = $skdp['alasan2'];
                  $data['result']['rtl1'] = $skdp['rtl1'];
                  $data['result']['rtl2'] = $skdp['rtl2'];
              }else{
                  $data['status'] = 'err';
                  $data['result'] = '';
              }
